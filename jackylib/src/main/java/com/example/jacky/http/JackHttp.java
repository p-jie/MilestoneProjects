...
package com.example.jacky.http;
...
import com.example.jacky.http.model.HttpParams;
import com.example.jacky.http.request.DeleteRequest;
import com.example.jacky.http.request.GetRequest;
import com.example.jacky.http.request.HeadRequest;
import com.example.jacky.http.request.OptionsRequest;
import com.example.jacky.http.request.PatchRequest;
import com.example.jacky.http.request.PostRequest;
import com.example.jacky.http.request.PutRequest;
import com.example.jacky.http.request.TraceRequest;
import com.example.jacky.http.utils.HttpUtils;
...
/**
 * Created by Jacky on 2018/3/4.
 */
public class JackHttp {
    ...
    public static JackHttp getInstance() {
        return JackHttpHolder.holder;
    }

    private static class JackHttpHolder {
        private static JackHttp holder = new JackHttp();
    }
    ...
    public JackHttp init(Application app) {
        context = app;
        return this;
    }
    ...
    public Context getContext() {
        HttpUtils.checkNotNull(context, "please call JackHttp.getInstance().init() first in application!");
        return context;
    }
    ...
    public OkHttpClient getOkHttpClient() {
        HttpUtils.checkNotNull(okHttpClient, "please call JackHttp.getInstance().setOkHttpClient() first in application!");
        return okHttpClient;
    }

    /** 必须设置 */
    public JackHttp setOkHttpClient(OkHttpClient okHttpClient) {
    ...
    /** 超时重试次数 */
    public JackHttp setRetryCount(int retryCount) {
    ...
    /** 全局的缓存模式 */
    public JackHttp setCacheMode(CacheMode cacheMode) {
    ...
    /** 全局的缓存过期时间 */
    public JackHttp setCacheTime(long cacheTime) {
    ...
    /** 添加全局公共请求参数 */
    public JackHttp addCommonParams(HttpParams commonParams) {
    ...
    /** 添加全局公共请求参数 */
    public JackHttp addCommonHeaders(HttpHeaders commonHeaders) {
