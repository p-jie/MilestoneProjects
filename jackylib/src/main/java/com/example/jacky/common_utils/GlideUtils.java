...
public class GlideUtils {
     ...
     */
    @SuppressWarnings("unchecked")
    public void LoadContextCircleBitmap(Context context, String path, ImageView imageView, int error) {
        Glide.with(context).load(path).bitmapTransform(new GlideCircleTransform(context)).error(error).into(imageView);
    }
...
//            Glide.with(context).load(path).bitmapTransform(new GlideRoundTransform(context,roundradius)).into(imageView);
            Glide.with(context).load(path).bitmapTransform(new CenterCrop(context), new GlideRoundTransform(context, roundradius)).into(imageView);
        }
    }
